...
function styleRunner(src, outputFilename, config, plugins) {
    return function () {
        let promise = gulp.src(src)
            ...
            .pipe(config.production ? plugins.cleanCSS({compatibility: 'ie8'}) : plugins.noop());
